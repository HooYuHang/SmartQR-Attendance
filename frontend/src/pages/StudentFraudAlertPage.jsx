...
export default function StudentFraudAlertPage() {
    ...
    const fetchFraudAttempts = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/fraud/student/${userInfo.sub}`,
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "20px" }}>
          <thead>
            <tr>
              ...
              <th style={{ border: "1px solid #fff", padding: "8px" }}>Date</th>
          ...
          <tbody>
            {frauds.map((f) => (
              <tr key={f._id}>
                ...
                <td style={{ border: "1px solid #fff", padding: "8px" }}>{f.classDate}</td>
